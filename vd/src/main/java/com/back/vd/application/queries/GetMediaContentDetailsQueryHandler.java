...
@Service
public class GetMediaContentDetailsQueryHandler {

    private final MediaContentRepositoryPort repositoryPort;

    public GetMediaContentDetailsQueryHandler(MediaContentRepositoryPort repositoryPort) {
        this.repositoryPort = repositoryPort;
    }

    @Transactional(readOnly = true)
    public MediaContentResponseDto execute(Long id) {
        MediaContent mc = repositoryPort.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("No se encontró el contenido multimedia con ID: " + id));
